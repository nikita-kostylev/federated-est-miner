...
class Trace(Sequence):
    ...
    def __eq__(self,other):
        if isinstance(self,Trace):
            self = tuple(event['concept:name'] for event in self)
        if isinstance(other,Trace):    
            other = tuple(event['concept:name'] for event in other)
        return self==other
    
    def __hash__(self):
        return hash(tuple(event['concept:name'] for event in self))
